...
| Infobulle d'un graphique multisérie | Elle ne nomme **pas** les séries : une ligne par série, réduite à sa valeur. Seule la pastille les distingue — et si `color-map` est posé, elle est fausse (BUG-022), donc l'infobulle ne porte plus aucune information de série. Les libellés sont pourtant transmis (`name` sur `<bar-chart>`) et la légende les affiche. À remonter chez `GouvernementFR/dsfr-chart`, pas ici (AM-086). |
