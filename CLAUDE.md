...
| Version de `dsfr-data` épinglée | Le dépôt est monté en **0.31.0** — les 65 pages la chargent, vérifié au `grep` le 2026-09-19. La 0.30.0 a été sautée : #35 a mené le banc de 0.29.1 directement en 0.31.0. **La 0.32.0 est publiée depuis le 2026-09-19 et aucune page ne la charge** : elle porte le mode dev de Lit (#899), la jointure `require-where` (#897), les overlays sous `databox` (#903), le débordement de l'accordéon a11y (#898) et `dsfr-data-repeat` (lot 2, #891). Vérifié dans le paquet publié (`npm pack dsfr-data@0.32.0`) : zéro occurrence de « Lit is in dev mode », zéro chemin `/development`, `dsfr-data-repeat` présent. **Tant que le banc n'est pas monté, il charge Lit en mode développement sur ses 65 pages** — les vérifications supplémentaires de Lit à chaque mise à jour de propriété. Une capacité peut être native, publiée, et **absente du bundle chargé** — auquel cas ses attributs sont ignorés **sans erreur console**. Avant de conclure à un manque : chercher dans le source, puis vérifier dans quelle version publiée l'attribut apparaît (`npm pack dsfr-data@<v>` + grep dans `package/dist/`). Quatre verdicts (ADR-109) : capacité native (faux problème) / native mais postérieure à la version épinglée (montée de version) / livrée sur `origin/main` mais pas encore publiée (attendre, et le dire ainsi) / absente du source (demande à la bibliothèque). |
...
| Jointure dont une entrée est en `require-where` | Peut rester sur « Chargement… » indéfiniment selon l'ordre du DOM : déclarer les autres entrées avant (BUG-015, en 0.28 comme en 0.29 — toujours ouvert). |
