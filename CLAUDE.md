...
| Conditionnelle dans un template | **Périmé — `{{#if}}` / `{{#unless}}` existent depuis 0.22.0** (#664), vérifié dans le bundle publié : la regex de bloc est absente de 0.20.0, présente dès 0.22.0. Le contournement CSS (interpoler dans `class`/`href`/`data-v`, masquer par `:empty`, `[href=""]`, `:has()`) n'est plus nécessaire une fois le dépôt monté en 0.23.0 ; le bloc « Annuaires » de `site.css` pourra être retiré. **Reste vrai** : il n'y a **pas de boucle** — la regex ne connaît que `if|unless`, donc pas de `{{#each}}`, donc pas moyen d'émettre un élément par valeur d'un champ multivalué (AM-039, requalifié).
