...
| Version de `dsfr-data` épinglée | Le dépôt est monté en **0.32.0** — les 65 pages la chargent, et c'est la dernière version publiée sur npm au 2026-09-19. La montée a consommé cinq constats, chacun avec sa preuve rejouée : BUG-021 (Lit en mode dev, #917), BUG-015 (jointure `require-where`, #897), BUG-019 (overlays sous `databox`, #903), BUG-018 (accordéon a11y, #898) et BUG-020 (avertissement #765 en O(N²), #915, livré en 0.31.0). ⚠️ **Une deuxième vague attend la publication de la 0.33.0** (PR de release `changeset-release/main`, dsfr-data#943) : elle porte `count-label` (#939 → AM-077), `share`/`share_percent` (#946 → AM-078), `map-summary` et `map-summary-field` (#947, #949 → AM-079), une valeur par défaut **et un libellé de valeur** sur les facettes (#940 → AM-084, AM-081), le groupe null et la dimension série dans le tableau a11y (#941 → AM-082, AM-085), deux contextes autour de l'URL (#942 → PG-028), `lazy` sur `dsfr-data-source` (#945 → AM-083) et le « 01 » comparé à un champ entier (#948 → PG-030). Ces capacités sont **sur `origin/main`, pas dans le bundle chargé** : les écrire aujourd'hui, c'est écrire des attributs ignorés **sans erreur console**. Une capacité peut être native, publiée, et absente du bundle chargé. Avant de conclure à un manque : chercher dans le source, puis vérifier dans quelle version publiée l'attribut apparaît (`npm pack dsfr-data@<v>` + grep dans `package/dist/`). Quatre verdicts (ADR-109) : capacité native (faux problème) / native mais postérieure à la version épinglée (montée de version) / livrée sur `origin/main` mais pas encore publiée (attendre, et le dire ainsi) / absente du source (demande à la bibliothèque). |
