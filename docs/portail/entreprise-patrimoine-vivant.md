...
|---|---|---|---|
| Recherche | `q` (full-text) | — | Autocomplétion à la saisie (debounce 1 s) : jusqu'à 10 suggestions « RAISON SOCIALE (département) » puis « + de 10 résultats, affinez votre recherche » ; message sans résultat en 4 lignes. Cliquer une suggestion pose `refine.raison_sociale` (ouvre la fiche). Entrée ou la loupe pose `q` **et remet tous les autres filtres à zéro**. |
...
- « Sur toutes ces entreprises labellisées : » puis deux cartes côte à côte, picto pièce :
  ...
  Les 4 KPI sont recalculés sur le contexte filtré (mais le panneau est masqué dès qu'un filtre est actif, donc en pratique on ne les voit qu'à 1267).
...
- Si le sous-ensemble filtré fait ≤ 150 entreprises, on saute directement aux points colorés quel que soit le zoom.
